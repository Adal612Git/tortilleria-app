...
const buildMonthlyStats = (sales: { date: string; value: number }[]) => {

  const map = new Map<string, { revenue: number; days: Set<string> }>();
  ...
  sales.forEach((item) => {
    ...
    if (entry) {
      ...
      map.set(month, { revenue: item.value, days });
...
export default function ReportsScreen() {
  ...
  const monthComparison = useMemo(() => {
    ...
    const monthStats = buildMonthlyStats(data.salesByDate);
    ...
    const getKey = (offset: number) => {
      ...
      return formatMonthKey(candidate);

    };

    const computeAverage = (key: string) => {
      ...
      return entry.revenue / entry.days.size;

    };

    const previousAverage = computeAverage(getKey(-1));

    const currentAverage = computeAverage(getKey(0));
    ...
    return { previousAverage, currentAverage, changePercent };

  }, [data.salesByDate]);



  const { previousAverage, currentAverage, changePercent } = monthComparison;
      ...
      <ScrollView
        ...
        <View style={styles.cardGrid}>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.momRowHeader}>

              <Text style={styles.muted}>Promedios mensuales</Text>

              <Text style={styles.sectionSubtitle}>Mes actual vs mes anterior</Text>

            </View>

            <View style={styles.momSection}>

              <Text style={styles.momLabel}>Promedio Ventas (Mes anterior)</Text>

              <Text style={styles.momValue}>

                {previousAverage !== null ? formatCurrency(previousAverage) : 'Sin datos previos'}
            ...
            <View style={styles.momSection}>
              ...
              <Text style={styles.momLabel}>Promedio Ventas (Mes actual)</Text>

              <View style={styles.momRow}>

                <Text style={styles.momValue}>

                  {currentAverage !== null ? formatCurrency(currentAverage) : 'Sin datos actuales'}
                ...
                {changePercent !== null ? (
                  ...
                  <View
                    style={[
                      styles.diffBadge,
                      changePercent >= 0 ? styles.diffBadgeUp : styles.diffBadgeDown,
                    ]}>

                    <Text
                      style={[
                        styles.diffIcon,
                        changePercent >= 0 ? styles.diffIconUp : styles.diffIconDown,
                      ]}>
                    ...
                    <Text
                      style={[
                        styles.diffText,
                        changePercent >= 0 ? styles.diffTextUp : styles.diffTextDown,
                      ]}>
                  ...
                  <Text style={styles.diffHelp}>Sin comparativa previa</Text>
        ...
        <View style={styles.sectionCard}>
          ...
          {chartData.length === 0 ? (
            ...
            <BarChart data={chartData} barColor="#7C3AED" valueFormatter={formatCurrency} />
            ...
            data.stockMovements.map((movement) => (
              ...
              <View key={movement.product} style={styles.stockRow}>
                ...
                <View style={styles.stockBadge}>
                  ...
                  <Text style={styles.stockBadgeText}>-{formatQuantity(movement.sold, 0)}</Text>

                </View>

                <View style={[styles.stockBadge, styles.stockBadgePositive]}>

                  <Text style={styles.stockBadgeText}>Inventario {formatQuantity(movement.remaining, 0)}</Text>
...
const styles = StyleSheet.create({
  ...
  momRowHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 },

  momSection: { marginBottom: 8 },

  momLabel: { color: '#475569', fontWeight: '600', marginBottom: 4 },

  momValue: { fontSize: 20, fontWeight: '800', color: '#0F172A' },

  momRow: { flexDirection: 'row', alignItems: 'center', marginTop: 4 },

  diffBadge: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 999, marginLeft: 12 },

  diffBadgeUp: { backgroundColor: '#DCFCE7' },

  diffBadgeDown: { backgroundColor: '#FEE2E2' },

  diffIcon: { fontSize: 12, marginRight: 4, lineHeight: 14 },

  diffIconUp: { color: '#047857' },

  diffIconDown: { color: '#B91C1C' },

  diffText: { fontSize: 12, fontWeight: '700' },

  diffTextUp: { color: '#047857' },

  diffTextDown: { color: '#B91C1C' },

  diffHelp: { color: '#94A3AF', fontSize: 12, marginLeft: 12 },
